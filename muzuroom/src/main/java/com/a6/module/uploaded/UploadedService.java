...
////...
    public int insert(UploadedDto uploadedDto) throws Exception { 
    	uploadedDao.insert(uploadedDto);
    	uploadFilesToS3(
    			uploadedDto.getUploadImg1()
    			, uploadedDto
    			, "infrBannerUploaded"
    			, uploadedDto.getType()
    			, uploadedDto.getSort()
    	...
    	public void insertUploaded(UploadedDto uploadedDto) throws Exception {
            // 업로드 처리 로직
            uploadedDao.insertUploaded(uploadedDto);
        }
    
    	
    	
    	public void uploadFilesToS3(MultipartFile[] multipartFiles, UploadedDto dto, String tableName, int type, int maxNumber, String pSeq, UploadedDao dao, AmazonS3Client amazonS3Client) throws Exception {
